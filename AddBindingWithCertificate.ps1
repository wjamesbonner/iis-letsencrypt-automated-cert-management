...
{
    # Create a http binding if it doesn't already exist
    if((Get-WebBinding -Protocol "http" -Name $siteName -HostHeader $domain).Count -eq 0) {
        New-WebBinding -Name $siteName -IPAddress "*" -Port 80 -HostHeader $domain -Protocol "http"
    }
    
    # Create a https binding if it doesn't already exist
    if((Get-WebBinding -Protocol "https" -Name $siteName -HostHeader $domain).Count -eq 0) {
